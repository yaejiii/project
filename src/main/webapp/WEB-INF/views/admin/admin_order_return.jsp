...
<body>
								...
								<form name="f" action="admin_order_return_search.do" method="post"
									>
    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-12">
          <table class="table">
            <thead>
            <th><a href  ="admin_order_return.do">전체목록으로</a></th>
              <tr>
            ...
            <tbody>
              <tr>
                ...
                <c:if test = "${empty listOrderReturn}">
                <td colspan = "8" align = "center">반품 목록이 포함된 주문내역이 존재하지 않습니다.</td>
              
                </c:if>
                
                <c:if test = "${not empty  listOrderReturn}">
                 <c:forEach var = "dto" items = "${ listOrderReturn}" >
               <td>   
...
<c:if test="${count >0}">
			...
			<c:if test="${mode != 'Y'}">
				<c:if test="${startPage > pageBlock}">
					[<a href="admin_order_return.do?pageNum=${startPage - 1}">이전</a>]	
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					[<a href="admin_order_return.do?pageNum=${i}">${i}</a>]		
				</c:forEach>
				<c:if test="${endPage < pageCount}">
					[<a href="admin_order_return.do?pageNum=${endPage+1}">다음</a>]
				</c:if>
			</c:if>	
</c:if>
</div>
